...
public class FCDState extends GameState {
    ...
    //instance Variables
    private ArrayList<Card> deck; //contains all undealt cards
    private ArrayList<Card> dealtCards; //contains dealt cards
    private String cardBack; //obsolete
    private int lastBet; //the last bet that was made in the game
    private int pot; //collective pot
    private int activePlayer; //the player who is to take their turn
    private int gameStage; //current phase of the game; ie, betting, throwing, showdown, wtc
    private int moveCount; //hols how many players have taken their turn during a gamestage
    private String lastAction; //last move action taken
    private ArrayList<Player> lobby; //list of players in the game
    ...
    public FCDState(FCDState gameState){
        ...
        for(int i = 0; i < 2; i++){
            this.lobby.add(i, new Player(gameState.getLobby().get(i), gameState));
        }
        for(Player p:gameState.getLobby()){
            index = gameState.getLobby().indexOf(p);
        ...
        for(int i = 0; i < gameState.getDeck().size(); i++){
            Card c = gameState.getDeck().get(i);
            this.deck.add(c);
        }
        ...
        this.lastAction = gameState.getLastAction();
    ...
    public void setLastAction(String string){this.lastAction = string; }

    public void setLastBet(int num){this.lastBet = num;}
